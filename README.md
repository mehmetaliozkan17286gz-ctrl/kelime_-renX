...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}
    ...
    /* Kelime Kartı yazısı özel sıralı animasyon:
       K - e - l - i - m - e -  boşluk - K - a - r - t - ı
       'ı' için önce nokta aşağıda, sonra 'i' animasyonu (şekil olarak canlandırma) */
...
<body>
  <div class="app">
    ...
    <div class="splash" aria-hidden="true">
      <div class="brand" role="img" aria-label="Kelime Kartı ile Kelime Öğren">
        <span class="k">K</span>
        <span class="e">e</span>
        <span class="l">l</span>
        <span class="i">i</span>
    ...
    <div class="topbar">
      <div class="mode-pill">
        <span id="modeLabel">Öğrenme modu</span>
        <span class="badge" id="badgeCount">Depo: 0</span>
      </div>
      <button class="btn ghost" id="settingsBtn">Ayarlar</button>
      <button class="btn primary" id="playBtn" title="Sesi çal">🔊 Dinle</button>
    </div>
    ...
    <div class="card-wrap">
      <div class="card" id="card" aria-live="polite">
        <div class="face front">
          ...
          <div class="sub">Türkçe, kartı çevirince görünür</div>
          <div class="actions">
            <button class="btn" id="flipBtn" title="Çevir">🔄 Çevir</button>
            <button class="btn warn" id="depotBtn" title="Depoya ekle">📦 Depoya ekle</button>
            <button class="btn" id="masterBtn" title="Öğrendim">✅ Öğrendim</button>
          </div>
        </div>
        <div class="face back">
          <div class="word" id="wordTr">...</div>
          <div class="sub">Türkçe karşılık</div>
          <div class="actions">
            <button class="btn" id="flipBackBtn" title="Geri çevir">↩️ Geri</button>
          </div>
        </div>
      </div>
    </div>
    ...
    <!-- Navigasyon -->
    <div class="nav">
      <button class="btn ghost" id="prevBtn">⬅️ Önceki</button>
      <button class="btn" id="toggleModeBtn">🔁 Depo / Öğrenme</button>
      <button class="btn ghost" id="nextBtn">Sonraki ➡️</button>
    </div>
    ...
    <div class="depot" id="depotPanel" hidden>
      ...
      <div style="margin-top:10px; display:flex; gap:8px; flex-wrap:wrap;">
        <button class="btn" id="reviewDepotBtn">🎯 Sadece depodan çalış</button>
        <button class="btn danger" id="clearDepotBtn">🗑️ Depoyu temizle</button>
      </div>
    </div>
  </div>
  ...
  <div class="modal" id="modal">
    <h3>Basit ayarlar</h3>
    <div class="row">
      <div>Ses (İngilizce söylet)</div>
      <input type="checkbox" class="switch" id="audioSwitch">
    </div>
    <div class="row">
      <div>İlerlemeyi sıfırla (öğrenilenler ve depo)</div>
      <button class="btn danger" id="resetBtn">Sıfırla</button>
    </div>
    <div class="row">
      <div>Depo kelimelerini görüntüle</div>
      <button class="btn" id="openDepotBtn">Depoyu aç</button>
    </div>
  ...
  <script>
    // ===============================
    // 300 temel İngilizce kelime veri seti
    // ===============================
    const WORDS = [
      // Meyveler (10)
      { en: "apple", tr: "elma" },
      ...
      { en: "uncle", tr: "amca" },
      { en: "cousin", tr: "kuzen" },
      { en: "grandmother", tr: "büyük anne" },
      { en: "grandfather", tr: "büyük baba" },
      { en: "family", tr: "aile" },
      ...
      { en: "ugly", tr: "çirkin" },
      { en: "noisy", tr: "gürültülü" },
      { en: "quiet", tr: "sessiz" },
      { en: "safe", tr: "güvenli" },
      { en: "dangerous", tr: "tehlikeli" },

      // Zamir / Belirteçler (20) -> toplam 300
      { en: "I", tr: "ben" },
      { en: "you", tr: "sen" },
      { en: "he", tr: "o (erkek)" },
      { en: "she", tr: "o (kadın)" },
    ...
    let audioOn = localStorage.getItem(LS_KEYS.audio) !== "false"; // varsayılan açık
    ...
    function render(){
      ...
      if(!item){
        wordEnEl.textContent = mode === "depot" ? "Depo boş" : "Tebrikler! Tüm kelimeleri işaretledin.";
      ...
      masterBtn.textContent = isMastered ? "✅ Öğrenildi" : "✅ Öğrendim";
      masterBtn.disabled = isMastered && mode !== "depot";

      const inDepot = depotSet.has(item._i);
      depotBtn.textContent = inDepot ? "📦 Depoda" : "📦 Depoya ekle";

      modeLabel.textContent = mode === "depot" ? "Depo modu" : "Öğrenme modu";
    }
    ...
    resetBtn.addEventListener("click", ()=>{
      if(!confirm("Tüm ilerlemeyi ve depoyu sıfırlamak istiyor musun?")) return;
      masteredSet = new Set();
      depotSet = new Set();
      currentIndex = 0;
      mode = "learn";
    ...
    reviewDepotBtn.addEventListener("click", ()=>{
      if(depotSet.size === 0){
        alert("Depo boş. Önce bazı kelimeleri depoya ekleyin.");
        return;
      }
    ...
    clearDepotBtn.addEventListener("click", ()=>{
      if(!confirm("Depodaki tüm kelimeleri kaldırmak istiyor musun?")) return;
      depotSet = new Set();
